...
export default function ClientApplicationDetail(){
  ...
  const load=()=>{setLoading(true);axios.get(`/api/applications/${id}`).then(r=>{setApp(r.data);initEf(r.data);}).finally(()=>setLoading(false));};
  useEffect(load,[id]);
  ...
  return(
    <Layout title={app.client?.clientId || '—'}>
      ...
      <div className="tabs-bar">
        {['overview','edit','documents','reports','feedback'].map(t=>(
          ...
          <div className="card-body">
            ...
            <div style={{border:'2px solid var(--primary)',borderRadius:10,marginBottom:18,overflow:'hidden'}}>
              ...
              <div style={{padding:'9px 14px',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
                <span style={{fontSize:12.5,flex:1}}>Percentage level of integration (decided by Q4A):</span>
