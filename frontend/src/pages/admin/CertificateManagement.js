...
async function generateCertificate(cert) {
  const fileName = `Certificate-${(cert.certNumber || 'Q4A').replace(/[^a-zA-Z0-9]/g, '_')}.jpg`;
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:16 }}>
        <FG label="Certificate Number" required>
          <div style={{ display:'flex', gap:6 }}>
            <input className="form-control" value={data.certNumber||''} onChange={e=>set('certNumber',e.target.value)} placeholder="Q4A/65A3B/0226" />
            {onGen && (
...
export default function CertificateManagement() {
  ...
  const [setting,     setSetting]     = useState({
    title:'Certificate of Registration', authority:'Q4A Management Private Limited',
