...
// ── Certificate number generator — format: Q4A/##L##L/MMYY ──
// matches  ^Q4A\/\d{2}[A-Z]\d{2}[A-Z]\/(0[1-9]|1[0-2])\d{2}$
...
async function generateCertNumber() {
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const num = `Q4A/${middle}/${mm}${yy}`;
    const exists = await Certificate.findOne({ certNumber: num }).select('_id').lean();
    if (!exists) return num;
  }
  return `Q4A/${randDigits(2)}${randLetter()}${randDigits(2)}${randLetter()}/${mm}${yy}`;
}
