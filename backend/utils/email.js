...
async function sendMail({ to, subject, html }) {
  ...
  if (brevoUser && brevoPass) {
    ...
    try {
      await t.sendMail({ from: `"Q4A CRM" <${brevoUser}>`, to, subject, html });
  ...
  if (gmailUser && gmailPass.length >= 16) {
    ...
    try {
      await t.verify();
      await t.sendMail({ from: `"Q4A CRM" <${gmailUser}>`, to, subject, html });
...
<body>
  <div class="wrapper">
    <div class="card">
      ...
      <!-- Header -->
      <div class="header">
        <p class="header-title">Q4A Management</p>
      ...
      <!-- Footer -->
      <div class="footer">
        <p>&copy; ${new Date().getFullYear()} Q4A Management &middot; Q4A CRM Platform. All rights reserved.</p>
      </div>
...
<table width="520" cellpadding="0" cellspacing="0"
  ...
  <tr>
    <td style="background:linear-gradient(135deg,#1565c0,#0d47a1);padding:34px 40px;text-align:center;">
      <div style="font-size:22px;font-weight:900;color:#fff;letter-spacing:.5px;">Q4A Management</div>
  ...
  <tr>
    <td style="background:#f8fafc;border-top:1px solid #e2e8f0;padding:20px 40px;text-align:center;">
      <p style="margin:0;font-size:12px;font-weight:600;color:#64748b;">Q4A Management &middot; Q4A CRM Platform</p>
...
const sendOtpEmail = ({ to, name, otp, expiresInMinutes = 10 }) =>
  sendMail({
    to,
    subject: `${otp} — Your Q4A Management Admin OTP`,
...
const sendWelcomeEmail = ({ to, name, clientId, email, password }) =>
  sendMail({
    to,
    subject: 'Welcome to Q4A CRM — Account Activated',
